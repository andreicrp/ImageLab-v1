...
if ($mode === 'async') {
    ...
    if ($jobId) {
        Response::json([
            ...
            'job_id' => $jobId,
            'status' => 'queued',
            'message' => 'Smart AI auto-enhancement task enqueued.'
        ]);
    } else {
        Response::error('Failed to enqueue auto-enhancement task.', 500);
    }
} else {
    // Sync mode
    ...
    $targetFilename = $sourceBase . "_ai_enhanced_" . time() . '.' . $sourceExt;
    $targetPath = Config::PROCESSED_PATH . $targetFilename;

    $aiService = new AIService();
    if (!$aiService->autoEnhance($sourcePath, $targetPath)) {
        Response::error('AI auto-enhancement failed.', 500);
    }
